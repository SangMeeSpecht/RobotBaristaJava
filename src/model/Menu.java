...
public class Menu {
    ...
    public void disableDrinks(Inventory inventory){
        for(int i = 0; i < menu.size(); i++){
            for(int i2 = 0; i2 < menu.get(i).recipe.size(); i2++){
                for(int i3 = 0; i3 < inventory.inventory.size(); i3++){
                    if(inventory.inventory.get(i3).ingredient == menu.get(i).recipe.get(i2).name){
                        if(inventory.inventory.get(i3).count < menu.get(i).recipe.get(i2).unit){
